...
new_df = df.groupby(['month']).agg({'actual_max_temp': 'mean', 'actual_min_temp': 'mean', 'actual_mean_temp': 'mean'}).reset_index()

# Preparing data
data = [go.Scatter(x=new_df['actual_max_temp'], y=new_df['actual_min_temp'], text=new_df['month'], mode='markers',
